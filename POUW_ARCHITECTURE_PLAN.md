...
### Problem: Iterating Through All NFT Holders is Gas-Intensive

The `_getHoldersAndAmounts()` function in the Gateway contract loops through all NFTs to find unique holders. For collections with 100+ NFTs, this can be expensive.

**Current Approach (On-Chain)**:
- Gateway calculates holders on-chain during `payAndRunAudit()`
- Works fine for small collections (<100 NFTs)
- Gets expensive for larger collections

**Enhanced Approach: Off-Chain Distributor (Recommended)**

Deploy a background service (on Render, Vercel, Railway, etc.) that:

1. **Watches for audit jobs** (listens to `JobCompleted` events from Gateway)
2. **Calculates NFT holder distribution off-chain** (much cheaper)
3. **Calls a modified Gateway function** with pre-calculated holder arrays

### Off-Chain Distributor Architecture:

```
┌─────────────────────────────────────────────────────────────────┐
│  Off-Chain Distributor Service (Node.js/TypeScript)             │
│  (Deployed on Render/Vercel/Railway)                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. Listen for JobCompleted events from Gateway                 │
│  2. Fetch all NFT holders from blockchain (off-chain query)     │
│  3. Calculate distribution (who gets how much)                  │
│  4. Call Gateway.distributeToHolders(holders[], amounts[])      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│  AIAuditAgentGateway (Modified)                                 │
│  • payAndRunAudit() → Emits JobCompleted event (no distribution)│
│  • distributeToHolders() → Receives pre-calculated arrays       │
└─────────────────────────────────────────────────────────────────┘
```

### Modified Gateway Pattern (Gas Optimized):

```solidity
// In AIAuditAgentGateway.sol

event JobCompleted(uint256 indexed jobId, address indexed user, uint256 payment);
event RewardsDistributed(uint256 indexed jobId, uint256 nftShare, uint256 treasury, uint256 burned, uint256 owner);

// Option 1: Auto-distribute on payment (current approach - simpler but more gas)
function payAndRunAudit() external payable nonReentrant {
    // Handle payment
    // ...
    
    totalJobsCompleted++;
    emit JobCompleted(totalJobsCompleted, msg.sender, msg.value);
    
    // Immediately distribute (gas-intensive for large collections)
    _distributeRewards();
}

// Option 2: Manual distribution via off-chain service (gas optimized)
function payAndRunAudit() external payable nonReentrant {
    // Handle payment
    // ...
    ...
    // Don't distribute yet - wait for off-chain service
}

/**
 * @notice Called by off-chain distributor with pre-calculated holder data
 * @dev Only callable by MCP_OPERATOR_ROLE (the distributor service)
 */
function distributeToHolders(
    uint256 jobId,
    address[] calldata holders,
    uint256[] calldata amounts
) external onlyRole(MCP_OPERATOR_ROLE) {
    require(jobId == totalJobsCompleted, "Wrong job ID");
    require(holders.length == amounts.length, "Length mismatch");
    
    // Verify total adds up correctly
    uint256 totalNFTShare = (REWARD_PER_JOB * NFT_SHARE_PERCENT) / 100;
    uint256 totalProvided = 0;
    for (uint256 i = 0; i < amounts.length; i++) {
        totalProvided += amounts[i];
    }
    require(totalProvided == totalNFTShare, "Incorrect total");
    
    // Distribute the rest immediately
    uint256 treasuryShare = (REWARD_PER_JOB * TREASURY_PERCENT) / 100;
    uint256 burnShare = (REWARD_PER_JOB * BURN_PERCENT) / 100;
    uint256 ownerShare = (REWARD_PER_JOB * OWNER_PERCENT) / 100;
    
    // 1. Allocate NFT holder rewards (using provided arrays)
    pouwContract.allocateRewards(address(nftCollection), holders, amounts);
    ...
    emit RewardsDistributed(jobId, totalNFTShare, treasuryShare, burnShare, ownerShare);
}
```

### Sample Off-Chain Distributor (Node.js):
...
const GATEWAY_ADDRESS = '0x...'; // AIAuditAgentGateway
const NFT_COLLECTION_ADDRESS = '0x...'; // SmartSentinelsAIAuditNFT

const account = privateKeyToAccount(`0x${DISTRIBUTOR_PRIVATE_KEY}`);
...
// Calculate holder distribution off-chain
async function calculateDistribution(nftShareTotal) {
    // Get total supply
    ...
    const rewardPerNFT = nftShareTotal / totalSupply;
    
    // Fetch all owners (off-chain, no gas cost)
    ...
    for (let i = 0; i < totalSupply; i++) {
        ...
        if (holderMap.has(owner)) {
            holderMap.set(owner, holderMap.get(owner) + rewardPerNFT);
        } else {
            holderMap.set(owner, rewardPerNFT);
        }
    }
    
    // Convert to arrays
    const holders = Array.from(holderMap.keys());
    const amounts = Array.from(holderMap.values());
    
    return { holders, amounts };
}

// Watch for new jobs and distribute
async function watchAndDistribute() {
    console.log('👀 Watching for audit jobs...');
    ...
    setInterval(async () => {
        ...
        if (currentBlock > lastBlock) {
            // Check for JobCompleted events
            const logs = await publicClient.getLogs({
                address: GATEWAY_ADDRESS,
                event: GATEWAY_ABI[0], // JobCompleted event
            ...
            for (const log of logs) {
                ...
                console.log(`🎯 Job ${jobId} completed! Calculating distribution...`);
                
                // Calculate distribution off-chain (free)
                const nftShare = 40.2 * 10**18; // 60% of 67 SSTL
                const { holders, amounts } = await calculateDistribution(BigInt(nftShare));
                
                console.log(`📊 Found ${holders.length} unique holders`);
                console.log(`📤 Calling distributeToHolders()...`);
                
                // Send distribution transaction
                const hash = await walletClient.writeContract({
                    address: GATEWAY_ADDRESS,
                    abi: GATEWAY_ABI,
                    functionName: 'distributeToHolders',
                    args: [jobId, holders, amounts]
                });
                
                console.log(`✅ Distribution TX sent: ${hash}`);
                
                const receipt = await publicClient.waitForTransactionReceipt({ hash });
                
                if (receipt.status === 'success') {
                    console.log(`🎉 Distribution successful for job ${jobId}!`);
                } else {
                    console.error(`❌ Distribution failed for job ${jobId}`);
                }
            }
...
1. **Render** (Recommended - always running)
...
4. **Vercel** (Not ideal)
   - Serverless, not for long-running processes
   - Better for webhooks/API endpoints

### Benefits:

✅ **Lower gas costs** - No on-chain looping through NFTs
✅ **Scalable** - Works with collections of any size
✅ **Reliable** - Service auto-restarts if it crashes
✅ **Fast** - Off-chain calculation is instant
✅ **Flexible** - Can add complex logic (e.g., staking multipliers) off-chain

### Security:

- Distributor wallet needs `MCP_OPERATOR_ROLE` on Gateway
- Distributor can only call `distributeToHolders()` with valid data
- Gateway verifies total amounts match expected distribution
- Gateway checks job ID to prevent double distribution
